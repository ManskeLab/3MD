...
def Component3_Decomp(dicomPath_50keV, dicomPath_65keV, Patient, Leg, filterimage=False, Correct_bone=True, Extend_seed=True):                
    ...
    reader50keV = vtk.vtkNIFTIImageReader() 
    reader50keV.SetFileName('/Users/mkuczyns/University of Calgary/Justin Tse - Gout_Data/GoutProject/images/Phantom/MSU_HA_PHANTOM_50KEV/50kev.nii') 
    reader50keV.SetDataScalarTypeToUnsignedShort()
    reader50keV.UpdateWholeExtent()
    reader50keV.Update()
    
    reader65keV = vtk.vtkNIFTIImageReader() 
    reader65keV.SetFileName('/Users/mkuczyns/University of Calgary/Justin Tse - Gout_Data/GoutProject/images/Phantom/MSU_HA_PHANTOM_65KEV/65kev.nii') 
    reader65keV.SetDataScalarTypeToUnsignedShort()
    reader65keV.UpdateWholeExtent()
    reader65keV.Update()
    
    # Image size should be the same between MSU and HA images
    ...
    if filterimage:
        ...
        filter_image.Update()
        nodes_vtk_array_50_HA = filter_image.GetOutput().GetPointData().GetArray(0)

        filter_image.SetInputConnection(reader65keV.GetOutputPort())
        filter_image.Update()
        nodes_vtk_array_65_HA = filter_image.GetOutput().GetPointData().GetArray(0)
    else:
    ...
    phantomROIsCSV = os.path.join(os.path.join(parentDir, 'input'), 'Phantom_ROIs_Aug2021.csv')
    ...
    sitk.WriteImage(im, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_ST.nii'))
    im = sitk.GetImageFromArray(np.rot90((Z[:,:,:,1] * 255).astype(np.uint8)))
    sitk.WriteImage(im, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_UA.nii'))  
    im = sitk.GetImageFromArray(np.rot90((Z[:,:,:,2] * 255).astype(np.uint8)))
    sitk.WriteImage(im, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_HA.nii'))
    ...
    sitk.WriteImage(im, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_50keV.nii'))
    ...
    sitk.WriteImage(im, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_65keV.nii'))
    im = sitk.GetImageFromArray(np.rot90((Array_Dual[:,:,:] * 255).astype(np.uint8)))
    sitk.WriteImage(im, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_Dual.nii'))
    im2 = sitk.InvertIntensity(im)
    sitk.WriteImage(im2, os.path.join(newPatientPath, 'Patient' + Patient + 'Leg_' + Leg + '_Dual_Sub.nii'))
